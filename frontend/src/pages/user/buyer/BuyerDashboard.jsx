...
//   const [activeTab, setActiveTab] = useState(tabFromUrl || "saved");

//   useEffect(() => { if (activeTab === "enquiries") fetchEnquiries(); }, [activeTab]);
//   useEffect(() => { if (tabFromUrl) setActiveTab(tabFromUrl); }, [tabFromUrl]);
//   useEffect(() => { fetchSavedProperties(); fetchRecentlyViewed(); fetchEnquiries(); }, []);
...
//   const handleTabChange = (id) => { setActiveTab(id); navigate(`/buyer?tab=${id}`); };
...
//               {formatPrice(savedProperties.reduce((s, p) => s + (p.price || 0), 0))}
//             </div>
...
//                           {p.image
//                             ? <img src={p.image} alt={p.title} className="bd-card-img" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
//                             : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><HomeIcon style={{ width: 32, height: 32, color: '#C4A97A', opacity: 0.4 }} /></div>
//                           }
//                           <button onClick={() => removeFromSaved(p._id)} style={{ position: 'absolute', top: 10, right: 10, background: 'white', border: 'none', width: 30, height: 30, borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
//                             <HeartSolid style={{ width: 14, height: 14, color: '#C4503C' }} />
//                           </button>
//                         </div>
//                         <div style={{ padding: '16px 18px' }}>
//                           <Link to={`/property/${p._id}`} style={{ textDecoration: 'none' }}>
//                             <h4 className="bd-sans" style={{ fontSize: '0.9rem', fontWeight: 600, color: '#1E1C18', marginBottom: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.title}</h4>
//                           </Link>
//                           <p className="bd-sans" style={{ fontSize: '0.72rem', color: '#8B7355', display: 'flex', alignItems: 'center', gap: 4, marginBottom: 14 }}>
...
//                             <div>
//                               <div className="bd-sans" style={{ fontSize: '0.55rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#A89880', marginBottom: 1 }}>Price</div>
//                               <div className="bd-serif" style={{ fontSize: '1.2rem', fontWeight: 500, color: '#1E1C18' }}>{formatPrice(p.price)}</div>
//                             </div>
//                             <Link to={`/property/${p._id}`} style={{ width: 32, height: 32, border: '1px solid rgba(139,115,85,0.3)', borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', transition: 'all 0.2s ease', color: '#8B7355' }}
...
//                               onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = 'rgba(139,115,85,0.3)'; e.currentTarget.querySelector('svg').style.color = '#8B7355'; }}
//                             >
...
//               {/* Quick Stats */}
//               <div style={{ background: 'white', border: '1px solid rgba(139,115,85,0.12)', borderRadius: 2, padding: '20px 22px' }}>
//                 <div className="bd-sans" style={{ fontSize: '0.58rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#A89880', marginBottom: 12 }}>Quick Stats</div>
//                 {[
//                   { label: 'Saved Properties',   value: savedProperties.length                             },
//                   { label: 'Active Enquiries',    value: enquiries.filter(e => e.status === 'pending').length },
//                   { label: 'Unread Enquiries',    value: enquiries.filter(e => !e.readByBuyer).length       },
//                   { label: 'Properties Viewed',   value: recentlyViewed.length                             },
//                 ].map(({ label, value }) => (
//                   <div key={label} className="bd-stat-row">
//                     <span className="bd-sans" style={{ fontSize: '0.78rem', color: '#8B7355', fontWeight: 300 }}>{label}</span>
//                     <span className="bd-serif" style={{ fontSize: '1.1rem', fontWeight: 500, color: '#1E1C18' }}>{value}</span>
//                   </div>
//                 ))}
//               </div>
...
//               {/* Recent Activity */}
//               <div style={{ background: 'white', border: '1px solid rgba(139,115,85,0.12)', borderRadius: 2, padding: '20px 22px' }}>
//                 <div className="bd-sans" style={{ fontSize: '0.58rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#A89880', marginBottom: 14 }}>Recent Activity</div>
//                 {[...enquiries.slice(0, 2), ...recentlyViewed.slice(0, 2)]
//                   .sort((a, b) => new Date(b.createdAt || b.viewedAt || 0) - new Date(a.createdAt || a.viewedAt || 0))
//                   .slice(0, 4)
//                   .map((item, i) => (
//                     <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 0', borderBottom: '1px solid rgba(139,115,85,0.08)' }}>
//                       <div style={{ width: 28, height: 28, border: '1px solid rgba(139,115,85,0.2)', borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
//                         {item.property
//                           ? <EyeIcon style={{ width: 12, height: 12, color: '#8B7355' }} />
//                           : <PaperAirplaneIcon style={{ width: 12, height: 12, color: '#8B7355' }} />
//                         }
//                       </div>
//                       <div style={{ minWidth: 0 }}>
//                         <div className="bd-sans" style={{ fontSize: '0.75rem', color: '#1E1C18', fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
//                           {item.property?.title || item.title || 'Property Enquiry'}
//                         </div>
//                         <div className="bd-sans" style={{ fontSize: '0.65rem', color: '#A89880', fontWeight: 300 }}>
//                           {formatDate(item.createdAt || item.viewedAt)}
//                         </div>
//                       </div>
//                     </div>
//                   ))}
//                 {enquiries.length === 0 && recentlyViewed.length === 0 && (
//                   <p className="bd-sans" style={{ fontSize: '0.78rem', color: '#A89880', fontWeight: 300 }}>No recent activity</p>
//                 )}
//               </div>

//               {/* CTA strip */}
//               <div style={{ background: '#1E1C18', borderRadius: 2, padding: '22px 20px', position: 'relative', overflow: 'hidden' }}>
//                 <div style={{ position: 'absolute', top: -30, right: -30, width: 90, height: 90, border: '1px solid rgba(196,169,122,0.15)', borderRadius: '50%', pointerEvents: 'none' }} />
//                 <div className="bd-sans" style={{ fontSize: '0.58rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#8B7355', marginBottom: 6 }}>Discover More</div>
//                 <h3 className="bd-serif" style={{ fontSize: '1.2rem', fontWeight: 400, color: '#F5F0E8', lineHeight: 1.2, marginBottom: 6 }}>
//                   Find Your <em style={{ fontStyle: 'italic', color: '#C4A97A' }}>Dream Home</em>
//                 </h3>
//                 <p className="bd-sans" style={{ fontSize: '0.72rem', color: '#6B6355', marginBottom: 18, fontWeight: 300 }}>Personalised recommendations based on your preferences.</p>
//                 <Link to="/properties" className="bd-btn-primary" style={{ textDecoration: 'none', background: '#F5F0E8', color: '#1E1C18' }}>
//                   Explore Properties
//                 </Link>
//               </div>
//             </div>
...
// BuyerDashboard.jsx - Updated with Overview Tab
import { useContext, useState, useEffect } from "react";
import { AuthContext } from "../../../context/AuthContext";
import { Link, useNavigate, useSearchParams } from "react-router-dom";
import API from "../../../api/axios";
...
export default function BuyerDashboard() {
  ...
  useEffect(() => { 
    ...
    fetchRecentlyViewed(); 
    fetchEnquiries(); 
    calculateStats(); // Calculate stats when data changes
  }, []);
...
useEffect(() => {
  ...
  if (enquiryId && enquiries.length > 0) {
    ...
    // Scroll to the specific enquiry or open details
  ...
  const calculateStats = () => {
    ...
    // Top property type (if you have type field in properties)
    const typeCount = {};
    recentlyViewed.forEach(item => {
      if (item.type) {
